...
create request automatically. Review the saved `error.json` before retrying:
for a confirmed v1 creation rejection, resolve its cause and start a new attempt
with a new key; the old key can replay the saved rejection. If acceptance is
uncertain, preserve the original request and key. Follow the
[v1 recovery table](/legacy-calls#recover-after-a-restart-or-lost-response)
