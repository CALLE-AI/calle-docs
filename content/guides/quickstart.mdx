...
uses the standard library, with no SDK or extra gems. Tested with Ruby 4.0.7 on
macOS. It runs the same [authorized greeting test](#run-a-complete-example)
described above; calls may consume credits.

From the cloned `calle-docs` directory, use a new private run directory outside
this repository:
...
# Retrieve the saved call after a restart or completed run; no new call
ruby examples/calls.rb resume ../calle-ruby-run
```

The saved-file and result-reading guidance above also applies. If no Call ID was
saved, keep the original request and follow [Calls recovery](/calls#recover-after-a-restart-or-lost-response).
For HTTP failures saved in `error.json`, use the [error recovery table](/errors#choose-the-next-action).
