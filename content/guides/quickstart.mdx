...
Outbound calling is not available in every country or region. Confirm that CALL-E supports your intended destination before building an integration. A phone number can be valid E.164 and still be rejected with `unsupported_region`; see [Errors](/errors) for recovery guidance.
