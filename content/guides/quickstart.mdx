...
Outbound calling is not available in every country or region. Check
[Supported Regions](/supported-regions) before building an integration. A phone
number can be valid E.164 and still be rejected with `unsupported_region`; see
[Errors](/errors) for recovery guidance.
