...
For Goal Run creation, `goal_not_published` means an active Goal has no published RunSpec, and `goal_not_executable` means the Goal is draft, paused, or retired. Existing Goal Run ids remain readable after a lifecycle change. [Contact enrollment](/goal-contacts) accepts draft and paused outbound Goals without publishing or resuming them.
