...
| Verified Calls test preview | Wait for a non-null `result` or `error`, not just terminal `status`. See [preview availability](/calls#preview-availability). |
| Planned Calls and Goal Run response additions | Poll while `result_status` is `pending`. `call_outcome` separates ordinary telephone outcomes from technical errors. These additions are not part of the verified test deployment. |
