...
`account_concurrency_exceeded` (HTTP 429) means the account already has its allowed number of active default-line tasks. The same account shares capacity across API keys, MCP, and Dashboard. Wait for the active task to finish before starting a new attempt.
