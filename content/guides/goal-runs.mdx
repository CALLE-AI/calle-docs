---
title: Goal Runs
description: Start outbound calls from a published Goal with one phone number and schema-validated variables.
---
...
For TypeScript field names and a configuration-reading example, see [SDK examples](#sdk-examples).
...
Save `GoalRun.id` from the response as `GOAL_RUN_ID`. The `Location` response header contains the same resource path, and `Retry-After` suggests when to begin polling. The response deliberately omits `phone` and `variables`; keep your own correlation between the idempotency key and order record. `call_id` is the selected calling-call identifier when that persisted fact is available, or `null` otherwise. It is not an answered-call signal.
...
The response examples in this section include the [unreleased response additions](/calls#preview-availability).
Confirm that contract is deployed before using `result_status`, `call_outcome`
or `transcript`. Without those additions, wait for a non-null `result` or `error`.

With the additions deployed, stop polling when `result_status` is no longer `pending`. `call_outcome` describes the telephone outcome separately. No-answer, busy and declined outcomes use `status: completed` and do not populate `error`. A completed execution can still have a pending business result.
...
| REST / Python | TypeScript |
| --- | --- |
| `published_run_spec` | `publishedRunSpec` |
| `input_schema` | `inputSchema` |
| `result_schema` | `resultSchema` |

Read the published configuration before starting a Run:

```ts
const goal = await client.goals.get("goal_delivery_confirmation");
const publishedRunSpec = goal.publishedRunSpec;
console.log(publishedRunSpec.version);
console.log(publishedRunSpec.inputSchema);
console.log(publishedRunSpec.resultSchema);
```

A successful `client.goals.get(...)` returns a Goal with a required
`publishedRunSpec`. Accessing `goal.published_run_spec` is a TypeScript type error;
in JavaScript, it returns `undefined` because that field name is absent. This
does not mean the Goal is unpublished. For an active Goal with no published
RunSpec, `get(...)` throws a `CalleAPIError` with `code === "goal_not_published"`.
See [Common integration errors](#common-errors) for other failures.

The SDK preserves property names inside the schemas and `variables`, such as
`customer_name`; use the names defined by the published input schema.

List Goals, start a Run, and wait for its result with `client.goals`:
