...
Send the API key as a bearer token. This read-only request lists up to one
executable published Goal without starting a call:
...
`-i` includes the HTTP status and response headers. `200 OK` confirms access to
this endpoint, even when the `data` array is empty. See [Auth errors](#auth-errors)
for the meanings of `401` and `403`.
