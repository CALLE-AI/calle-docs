...
Create and manage API keys in the [CALL-E dashboard](https://dashboard.heycall-e.com/account/api-keys).
...
```bash
curl -i "https://api.heycall-e.com/v1/goals?limit=1" \
  -H "Authorization: Bearer $CALLE_API_KEY"
```

`-i` shows the HTTP status and headers. `200 OK` means the request succeeded,
even if `data` is empty. See [Auth errors](#auth-errors) for `401` and `403`.
...
The [complete Python call example](https://github.com/CALLE-AI/calle-docs/blob/main/examples/calls.py)
reads `CALLE_BASE_URL` for both `start` and `resume`. If it is unset or empty,
the example keeps the SDK's default URL. Use the same API key and environment
when resuming a saved call. Keep test and production API keys separate, and do
not reuse idempotency keys across environments.
