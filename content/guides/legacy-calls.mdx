...
These rules apply to `POST /v1/calls`. For `/v2/calls`, use the
[current Calls retry rules](/calls#idempotency).

An `Idempotency-Key` identifies one creation attempt, including a saved rejection.
Choose how to recover before retrying: read an existing Call ID, reuse the key
when acceptance is uncertain, or use a new key for an intentionally new attempt
after a confirmed rejection and resolution of its cause.

For example, if v1 saves a `429 account_concurrency_exceeded` rejection, retrying
with that key returns the saved error even after capacity becomes available.
The repeated error does not establish that the account is still at its limit.
Once capacity is available, save a new key for the new attempt and retain it
for any retries whose acceptance is uncertain.
