...
<aside className="calling-mode-guidance" aria-label="Calling mode guidance">

The **one-shot Calls API** (`POST /v2/calls`) is designed for quick integration, development, and testing. Each request creates one phone task, with no published Goal needed.

For batch and large-scale calling, we recommend [Goal Runs](/goal-runs), which will cost **50% less** under the [upcoming billing model](/billing). This pricing is not yet in effect.
