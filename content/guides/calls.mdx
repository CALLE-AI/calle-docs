...
| Purpose | HTTP and Python | TypeScript |
| --- | --- | --- |
| Calls API `call_id` | Top-level `id` (`call_...`) | `call.id` |
| Dashboard Call Records | `recipients[].attempts[].provider_call_id` | `call.recipients[i].attempts[j].providerCallId` |
