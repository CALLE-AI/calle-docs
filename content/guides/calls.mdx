...
The top-level call task `id` (for example, `call_...`) is the Developer API
resource identifier. Use it with the GET endpoints above and to correlate
events and webhooks.

To match an API attempt with Dashboard Call Records, use its
`recipients[].attempts[].provider_call_id` when available. A call task can
contain multiple recipients or attempts, so match each attempt separately. Do
not pass a `provider_call_id` to `/v1/calls/{call_id}`.
