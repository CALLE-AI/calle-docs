...
| Original request and idempotency key, but no Call ID | Repeat the create request with the same key and unchanged body. If the original request was accepted, save the returned Call ID. A saved creation rejection is replayed as an error instead. |
...
After resolving a confirmed creation rejection, use a new key for an
intentionally new attempt. An uncertain response alone is not evidence of a
rejection.
...
| Situation | Request and key to use |
| --- | --- |
| Confirmed missing-information creation rejection; task corrected | Save the corrected request with a new key before sending it. |
| Retry of that corrected request after a lost response | Keep the corrected request and its new key unchanged. |
| Timeout or lost response without a definite rejection | Preserve and recover the original request and key. Do not switch to a new key. |
| Call ID already saved | Retrieve that call. Do not create another one to check its outcome. |
