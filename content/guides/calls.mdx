...
judgment, and `evidence` supports it. These fields are independent of your
...
`structured_result` against the call transcript. To distinguish a person from
IVR or voicemail, use the [custom answered_by example](#classify-the-final-endpoint)
alongside your business result. Keep `unknown` answers unresolved.
