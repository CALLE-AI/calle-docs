...
[Quickstart](/quickstart) to try it in the test environment.

Check [Regions & languages](/regions) for supported destinations and languages.

## Preview availability

This is a preview contract, not a production availability announcement.
Optional target hints, input validation, idempotent creation and detailed events
were verified in the test preview on September 22, 2026. Shared test deployments
can change; confirm the deployed contract before integrating.

The `call_outcome`, `result_status` and fixed `transcript` fields below are
**unreleased response additions**, not part of that verified deployment.
Until they are deployed, follow the [Quickstart polling rule](/quickstart#wait-for-the-result):
wait for a non-null `result` or `error`. The result-outcome and transcript sections
describe the intended contract, not a guarantee about the current test response.

## Call ID and Billing

The response adds a fixed `call_id` field for matching a telephone call to the
**Call ID** shown in Billing. This field is a local preview addition and has not
yet been deployed.
...
### Planned result outcomes

The following outcome/readiness behavior belongs to the
[unreleased response additions](#preview-availability).
...
This field belongs to the [unreleased response additions](#preview-availability).
...
- Attempt details and provider identifiers are not exposed by the Calls response.
