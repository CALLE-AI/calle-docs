...
A default-line task supports one phone number. Batch tasks require an eligible purchased number selected as the account default outbound number. The account normally permits one active default-line task, including planning. Waiting for user input or confirmation releases planning capacity.
